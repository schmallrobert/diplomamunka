...
ggplot(tfr_df_eredeti, aes(x = evek_tfr, y = tfr_total_eredeti, color = I("darkslategray"))) + 
  geom_line(size = 1.5) + 
  labs(title = "Termékenységi ráta alakulása", x = "Év", y = "Termékenységi ráta")

ggplot(tfr_df_eredeti, aes(x = evek_tfr, y = tfr_total_eredeti, color = I("darkslategray"))) + 
  geom_line(size = 1.5) +   
  labs(title = "Termékenységi ráta alakulása", x = "Év", y = "Termékenységi ráta") +
  geom_hline(yintercept = 1.7, color = "darkslategray3",size = 1, linetype = "dashed")
...
korhatar_male <- c(rep(65, 38)) #c(rep(65, 10), rep(66,8), rep(67, 8), rep(68, 8), rep(69, 4))
...
korhatar_female <- c(rep(65, 38)) #c(rep(65, 10), rep(66,8), rep(67, 8), rep(68, 8), rep(69, 4))
...
```
```{r}
forgatokonyvek <- read_excel("forgatokonyvek.xlsx", 
    sheet = "forgatokonyvek_TDK")

ggplot(forgatokonyvek, aes(x = ev)) + 
  geom_line(aes(y = jobbgazd, color = "Kedvezőbb gazdaság"), size = 1) +
  geom_line(aes(y = rosszgazd, color = "Kedvezőtlenebb gazdaság"), size = 1) +
  geom_line(aes(y = jobbterm, color = "Kedvezőbb termékenység"), size = 1) +
  geom_line(aes(y = rosszterm, color = "Kedvezőtlenebb termékenység"), size = 1) +
  geom_line(aes(y = jar, color = "Magasabb járulék"), size = 1) +
  geom_line(aes(y = korhatar, color = "Magasabb korhatár"), size = 1) +
  geom_line(aes(y = alap, color = "Alap"), size = 1) +
  labs(title = "Forgatókönyvek", x = "Év", y = "Egyenleg (GDP %)") +
  scale_y_continuous(labels = scales::percent, breaks = c(-0.2, -0.15, -0.1, -0.05, 0), limits = c(-0.175, 0)) +
  scale_x_continuous(breaks = c(2023, 2030, 2040, 2050, 2060)) +
  scale_color_manual(name = "Jelmagyarázat", values = c( "Alap" = "red3", "Kedvezőbb gazdaság" = "green4", "Kedvezőtlenebb gazdaság" = "orange", "Kedvezőbb termékenység" = "black", "Kedvezőtlenebb termékenység" = "darkslategray3", "Magasabb járulék" = "steelblue3", "Magasabb korhatár" = "salmon2"))
```



```{r}
helyettesitesi <- as.vector(atlag_nyugdij_brutto_fc/brutto_berek_df[65,-1:-2])
helyettesitesi_evek <- 2023:2060
helyettesitesi_df <- data.frame( t(helyettesitesi), helyettesitesi_evek)
colnames(helyettesitesi_df) <- c("rata", "evek")

ggplot(helyettesitesi_df) + 
  geom_line(aes(x = helyettesitesi_evek, y = rata), color = "red3", size = 1.5) +
  labs(title = "Helyettesítési ráta alakulása", y = "Helyettesítési ráta", x = "Évek") +
  scale_x_continuous(breaks = c(2023, 2030, 2040, 2050, 2060)) +
  scale_y_continuous(breaks = c(0.2, 0.25, 0.3, 0.35, 0.4), limits = c(0.2, 0.4))
```
